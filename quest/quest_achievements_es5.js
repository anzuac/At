...
(function(){
  ...
  var LEVEL_TICKET_ITEM = "被動能力券";   // 等級成就相關，被動能力券
...
// —— 讀玩家（基礎 + 核心 + 潛能；攻防不吃技能乘算）—— //
  function readSnapshot(){
    ...
    var skl  = p.skillBonus || {};
    var pot  = p.PotentialBonus || {};
    var jobs = window.jobs || {};

    var jobKey = (p.job || "").toLowerCase();
    var jm = (jobs[jobKey] && jobs[jobKey].statMultipliers) || {
      str: 1,
      agi: 1,
      int: 1,
      luck: 1
    ...
    // ---- 主屬性：基礎 + core + 潛能 ----
    var totalStr = Number(base.str || 0) + Number(core.str || 0) + Number(pot.str || 0);
    var totalAgi = Number(base.agi || 0) + Number(core.agi || 0) + Number(pot.agi || 0);
    var totalInt = Number(base.int || 0) + Number(core.int || 0) + Number(pot.int || 0);
    var totalLuk = Number(base.luk || 0) + Number(core.luk || 0) + Number(pot.luk || 0);
    ...
    var aspdGain = Math.max(0, aspdRaw - 1);
    ...
    // ---- 擊殺 / 累傷計數維持原本 state ----
    var kills       = Number(state.counters.kills       || 0);
    var eliteKills  = Number(state.counters.eliteKills  || 0);
    var bossKills   = Number(state.counters.bossKills   || 0);
    var totalDamage = Number(state.counters.totalDamage || 0);
  ...
  function giveLevelTickets(prevLv, newLv){
    ...
    // 6~200：每升 1 等 +1 張
    var a = Math.max(prevLv, 5);
    var b = Math.min(newLv, 200);
    if (b > a){
      tickets += (b - a);
    }
  ...
  window.Achievements = {
    ...
    renderInto: function(container){
      ...
      var ticketUnlocked = ticketsUnlockedByLevel(snap.level);
      var ticketMax = 200;
      var ticketPct = (ticketMax > 0) ? (ticketUnlocked / ticketMax) : 0;
