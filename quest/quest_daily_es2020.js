// quest_online_time_es2020.js — 在線時間任務（每日/每週）V1（SaveHub 中央存檔）
(function () {
  "use strict";
  ...
  const WEEKLY_MINUTES = 360; // 6 hr
  const WEEKLY_REWARD = { star: 50 };
  ...
  function normalize(s){
    ...
    ["m5","m10","m20","m30","m60"].forEach((k) =>{ s.dailyClaimed[k] = !!s.dailyClaimed[k]; });
    s.weeklyClaimed = !!s.weeklyClaimed;
    return s;
  }
  if (SH && typeof SH.registerNamespaces==="function"){
    SH.registerNamespaces({ [NS]: { version: 1, migrate(o){ return normalize(o); } } });
  }
  ...
  function grant(rew){
    const st = Math.max(0, Number(rew.star||0)|0);
    const gm = Math.max(0, Number(rew.gem||0)|0);
    if (st>0){
  ...
  function tickLoop(){
    const now = Date.now();
    const delta = Math.max(0, Math.floor((now - lastTickAt)/1000));
    lastTickAt = now;
  ...
  document.addEventListener("visibilitychange", () =>{ if (document.visibilityState === "visible") startTick(); else stopTick(); });
  ...
  function scheduleRender(){
    ...
    __renderTimer = setTimeout(() =>{ __renderTimer=null; try { renderActiveTab(); } catch(e){} }, 80);
  }

  function renderDaily(){
    const box = document.getElementById("questContent"); if (!box) return;
    const s = state; // 使用記憶體中狀態
    let html = "";
    html += '<div style="margin-bottom:8px;color:#9aa">今日在線：<b>'+fmtHMS(s.daySecs)+'</b></div>';
    DAILY_THRESHOLDS.forEach((t) =>{
      const need = t.minutes*60;
      const done = s.daySecs >= need;
      const claimed = !!s.dailyClaimed[t.key];
      html += '<div style="border:1px solid #334155;background:#0b1220;border-radius:10px;padding:10px;margin-bottom:8px;">' +
                ...
                '</div>';
      const cur = Math.min(need, s.daySecs);
    ...
    // 綁定每日領取
    const btns = box.querySelectorAll ? box.querySelectorAll("[data-claim-d]") : [];
    for (let i=0;i<btns.length;i++){
      (function(b){
        b.onclick = function(){
          const k = b.getAttribute("data-claim-d");
          state = load(); state = normalize(state);
          const def = DAILY_THRESHOLDS.find((x) =>{ return x.key===k; });
          if (!def) return;
  ...
  function renderWeekly(){
    const box = document.getElementById("questContent"); if (!box) return;
    const s = state;
    const need = WEEKLY_MINUTES*60;
    const cur = Math.min(need, s.weekSecs);
    const pct = need>0 ? Math.floor(cur/need*100) : 100;
    let html = '';
    ...
    const btn = document.getElementById("btnWeeklyClaim");
    if (btn){
  ...
  function renderActiveTab(){
    try{
      const tab = (QuestCore.getActiveTab && QuestCore.getActiveTab()) || "";
  ...
  function init(){
    ...
    // 綁定分頁切換按鈕（若存在）
    const btnD = document.getElementById("tabDaily");   if (btnD) btnD.onclick = function(){ QuestCore.setTab("daily"); };
    const btnW = document.getElementById("tabWeekly");  if (btnW) btnW.onclick = function(){ QuestCore.setTab("weekly"); };
