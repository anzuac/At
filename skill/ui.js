// =======================
// ui_skills_full.js — 技能彈窗 + SkillsHub 分頁（最終卡片版, ES5, SaveHub 版 Only）
...
(function(w, d){
  ...
  function getSkillsStateFromHub(){
    ...
    var ns = w.SaveHub.get("skills", { _ver: 1, levels: {}, tiers: {}, auto: {} }) || {};
    ns.levels = ns.levels || {};
    ns.tiers  = ns.tiers  || {};
    ns.auto   = ns.auto   || {};
    return ns;
  ...
  function applySkillsStateFromHub(){
    ...
    for (var i = 0; i < w.skills.length; i++){
      var s = w.skills[i];
  ...
  function saveOneSkillToHub(skill){
    ...
    var ns = w.SaveHub.getOrInit("skills", { _ver: 1, levels: {}, tiers: {}, auto: {} }) || {};
    ns.levels = ns.levels || {};
    ns.tiers  = ns.tiers  || {};
    ns.auto   = ns.auto   || {};
  ...
  w.renderSkillListInto = function(container, type) {
    ...
    for (var j=0;j<filtered.length;j++){
      (function(skill){
        var tier = getTier(skill);
        var lg = getLogic(skill, tier);

        // --- MP 顯示（base + grow*Lv；你要的「等級×1」） ---
        var baseMp = Number(tier && tier.mpCost != null ? tier.mpCost : (skill.mpCost || 0));
        var mpGrow = Number(lg.mpCostLevelGrowth || 0);
        if (!isFinite(mpGrow)) mpGrow = 0;
        var viewMP = baseMp + mpGrow * Math.max(1, (skill.level || 1));

        // --- CD 顯示（扣精通 / 支援 CD=0） ---
        var rawCD = Number(tier && tier.cooldown != null ? tier.cooldown : (skill.cooldown || 0));
        ...
        var viewCD = cdZero ? 0 : Math.max(0, rawCD - cdRed);
        var viewCDStr = (Math.round(viewCD * 10) / 10).toFixed(1);
        ...
        var pillCD = d.createElement('span'); pillCD.className = 'sk-pill info'; pillCD.textContent = "CD " + viewCDStr + "s";
        ...
        var rawDesc = (typeof skill.getDescription === 'function') ? skill.getDescription() : (skill.description || "");
        ...
        var maxLv = Number(skill.maxLevel || 20);
        var canUpgrade = (skill.level || 1) < maxLv && own >= tkCost;
