// =======================
// explore.js — 探索（多隊 / 每日上限 / 升級 / 重置券）ES5
...
var EXPLORE_TABLE=[
  ...
  if (w.SaveHub){
    var spec = {}; spec[NS] = {
      version: SCHEMA_VER,
      migrate: function(old){ return old || {}; }
    };
  ...
  (function migrateFill(){
    ...
    if (!state.squads || !state.squads.length){
      state.squads = [];
      for (var i=0;i<SQUADS_BASE;i++) state.squads.push(newSquad(i));
    }
    for (var j=0;j<state.squads.length;j++){
      var q = state.squads[j];
      q.id = toInt(q.id || j);
  ...
  // === 可見性守門（避免覆蓋其他分頁的畫面）===
  var _mounted = false, _container=null;
