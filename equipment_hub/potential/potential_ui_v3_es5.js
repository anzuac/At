// =======================================================
// potential_ui_v2_es5.js — 潛能 UI（直立版：收合 + 機率表彈窗 + 保底移到洗方塊區）
...
(function (w, d) {
      ...
      for (var k in props) if (Object.prototype.hasOwnProperty.call(props, k)) {
        if (k === "style") {
          ...
          if (typeof s === "string") {
            ...
            for (var sk in s) if (Object.prototype.hasOwnProperty.call(s, sk)) node.style[sk] = s[sk];
          }
  ...
  w.potui2_openCombineModal = function (which) {
    ...
    var core = w.PotentialCoreV2 || w.PotentialCoreV1;
    ...
    var itemName = (which === "add") ? core.ITEM_ADD_COMBINE : core.ITEM_MAIN_COMBINE;
    ...
    function rerenderQty() {
      var q = modal.querySelector("#potui2_combine_qty");
    ...
    function renderBody() {
      ...
      // 三排能力顯示（抽選後會高亮）
      var linesBox = el("div", { id: "potui2_combine_lines", style: "margin:10px 0;" }, []);
      try {
        var st0 = core.getState && core.getState();
        var node0 = st0 && st0.pots && st0.pots[slot] ? st0.pots[slot][(which === "add") ? "add" : "main"] : null;
      ...
      var btnUse = el("button", { id: "potui2_combine_use", style: "width:100%;border:0;border-radius:12px;padding:10px 12px;background:rgba(80,140,255,.22);color:#fff;font-weight:800;cursor:pointer;", text: "使用結合方塊" });

      var btnRow = el("div", { style: "display:flex;gap:10px;margin-top:10px;" }, [
      ...
      var rules = el("div", { style: "margin-top:10px;font-size:12.5px;opacity:.85;line-height:1.35;", text: ruleText });
      ...
      // handlers
      function doDraw() {
        var r = (which === "add") ? core.drawAddCombine(slot) : core.drawMainCombine(slot);
        if (!r || !r.ok) {
        ...
        try {
          var st2 = core.getState && core.getState();
          var node2 = st2 && st2.pots && st2.pots[slot] ? st2.pots[slot][(which === "add") ? "add" : "main"] : null;
      ...
      function doConfirm() {
        ...
        var rr = (which === "add") ? core.confirmAddCombine(slot, selectedLineIndex) : core.confirmMainCombine(slot, selectedLineIndex);
        if (!rr || !rr.ok) { alert("確定使用失敗"); return; }
        // ✅ 不關閉視窗：更新顯示、清除本次抽選（需再次使用/重抽才可再確定）
        selectedLineIndex = null;
        chosen.textContent = "已確定使用完成。可再次按「使用」重新抽選一排。";
        try {
          var st3 = core.getState && core.getState();
          var node3 = st3 && st3.pots && st3.pots[slot] ? st3.pots[slot][(which === "add") ? "add" : "main"] : null;
    ...
    var cbtn = modal.querySelector("#potui2_combine_close_btn");
...
var _TIER_ORDER = ["特殊", "稀有", "罕見", "傳說", "唯一", "永恆"];
function tierIndex(t) {
  t = String(t || "特殊");
  for (var i = 0; i < _TIER_ORDER.length; i++) if (_TIER_ORDER[i] === t) return i;
  return 0;
}
...
function getSlotTierInfo(st, slot) {
  ...
  if (st && st.pots && st.pots[slot]) {
    var node = st.pots[slot];
    mt = node.main ? String(node.main.tier || "特殊") : "特殊";
    at = node.add  ? String(node.add.tier  || "特殊") : "特殊";
  }
  var hi = maxTier(mt, at);
  var neon = isEternal(mt) && isEternal(at);
  return { mt: mt, at: at, hi: hi, neon: neon };
}
  ...
  function renderLines3(container, node, glow, selectedIndex) {
    if (!container) return;
    container.innerHTML = "";
    var lines = (node && node.lines) ? node.lines : [];
    selectedIndex = Number(selectedIndex || 0); // 1~3
    for (var i = 0; i < 3; i++) {
      var L = lines[i] || null;
      var t = lineToText(L);
      var isSel = (selectedIndex === (i + 1));
...
// -----------------------------
  ...
  function getRuleUI(which, tier) {
    ...
    which = (which === "add") ? "add" : "main";
    try {
      var core = w.PotentialCoreV2;
      if (core && typeof core.getTierRuleTable === "function") {
        var tbl = core.getTierRuleTable(which);
        var r = (tbl && tbl[tier]) ? tbl[tier] : (tbl && tbl["特殊"]);
        if (r) return { pity: n(r.pity), upChance: n(r.upChance) };
      }
    } catch (e) {}
    // fallback（理論上不會用到）
    var fb = { "特殊": { pity: 50, upChance: 0.05 } };
    var rf = fb[tier] || fb["特殊"];
...
function statLabelZH(stat) {
  ...
  function lineToText(line) {
    ...
    if (meta && meta.stats) {
      ...
      if (id === "ACC_EXP_MESO_DROP") {
        var exp = (s.expRate != null) ? ("經驗值 " + pctVal(n(s.expRate))) : "";
        var meso = (s.mesoRate != null) ? ("金幣 " + pctVal(n(s.mesoRate))) : "";
        var drop = (s.dropRate != null) ? ("掉落 " + pctVal(n(s.dropRate))) : "";
        var parts = [];
        if (exp) parts.push(exp);
        if (meso) parts.push(meso);
        if (drop) parts.push(drop);
        return { tier: tier, desc: "三抽一", val: parts.join(" / ") };
      }
      // fallback：把 bundle 直接 JSON 顯示（方便除錯）
      return { tier: tier, desc: id || "特效", val: JSON.stringify(s) };
    }

    // 主屬性（會帶 meta.stat = str/dex/int/luk）
    if (id.indexOf("MAINSTAT_FLAT_") === 0) return { tier: tier, desc: statLabelZH(stat) || "主屬性", val: flatVal(v) };
    if (id.indexOf("MAINSTAT_PCT_") === 0)  return { tier: tier, desc: statLabelZH(stat) || "主屬性", val: pctVal(v) };

    // 全屬性
    if (id.indexOf("ALLSTAT_FLAT_") === 0) return { tier: tier, desc: "全屬性", val: flatVal(v) };
    if (id.indexOf("ALLSTAT_PCT_") === 0)  return { tier: tier, desc: "全屬性", val: pctVal(v) };

    // 其他：統一用 meta.stat 來顯示（核心新版多數都會帶）
    if (stat) {
      var label = statLabelZH(stat) || statLabel(stat) || stat;
      // value < 1 視為百分比（0.30 = 30%）；>=1 視為固定值
      var isPct = (Math.abs(v) > 0 && Math.abs(v) < 1);
  ...
  // -----------------------------
  var SUMMARY_KEYS = ["str", "agi", "int", "luk", "atk", "def", "hp", "mp"];

  function collectBonusForSlot(player, slot) {
    player = player || w.player;
    var out = {};
    for (var i = 0; i < SUMMARY_KEYS.length; i++) out[SUMMARY_KEYS[i]] = 0;
  ...
  function collectBonusAll(player) {
    ...
    if (player.PotentialBonus.uiTotal && typeof player.PotentialBonus.uiTotal === "object") {
      var ut = player.PotentialBonus.uiTotal;
      for (var k1 in ut) if (Object.prototype.hasOwnProperty.call(ut, k1)) {
    ...
    var all = bd.potential_all;
    if (all && typeof all === "object") {
      for (var k2 in all) if (Object.prototype.hasOwnProperty.call(all, k2)) {
    ...
    for (var key in bd) if (Object.prototype.hasOwnProperty.call(bd, key)) {
      if (key === "potential_all") continue;
      var obj = bd[key];
      if (!obj || typeof obj !== "object") continue;
      for (var kk in obj) if (Object.prototype.hasOwnProperty.call(obj, kk)) {
  ...
  function setRollButtonsEnabled(enabled) {
    var a = d.getElementById("potui2_main_roll");
    var b = d.getElementById("potui2_add_roll");
    var c = d.getElementById("potui2_main_combine");
    var d2 = d.getElementById("potui2_add_combine");
  ...
  function showCoreWarning(show) {
    var box = d.getElementById("potui2_core_warn");
    if (!box) return;
  ...
  var allSummaryMode = "all"; // all | pct | flat
  ...
  function applyFoldUI() {
    // all
    var allBody = d.getElementById("potui2_all_body");
    var allIcon = d.getElementById("potui2_all_icon");
    if (allBody) allBody.style.display = _fold.allOpen ? "block" : "none";
    if (allIcon) allIcon.textContent = _fold.allOpen ? "▴" : "▾";

    // single
    var sBody = d.getElementById("potui2_single_body");
    var sIcon = d.getElementById("potui2_single_icon");
  ...
  function openProbModal() {
    closeProbModal();
    var back = el("div", { id: "potui2_prob_backdrop" });
    var modal = el("div", { id: "potui2_prob_modal" });

    var header = el("div", { id: "potui2_prob_header" }, [
      el("div", { text: "機率表（升階 / 保底）" }),
      el("button", { className: "p2btn danger", id: "potui2_prob_close" }, ["關閉"])
    ]);

    var body = el("div", { id: "potui2_prob_body" });
    ...
    for (var i = 0; i < tiers.length; i++) {
      (function (t) {
        var rMain = getRuleUI("main", t);
        var rAdd  = getRuleUI("add", t);
        var mainTxt = (t === "永恆") ? "—" : (fmtPct(rMain.upChance, 3) + " / " + (rMain.pity ? (rMain.pity + "顆") : "無保底"));
        var addTxt  = (t === "永恆") ? "—" : (fmtPct(rAdd.upChance, 3) + " / " + (rAdd.pity ? (rAdd.pity + "顆") : "無保底"));
  ...
  function closeProbModal() {
    var b = d.getElementById("potui2_prob_backdrop");
    var m = d.getElementById("potui2_prob_modal");
    if (b) b.remove();
    if (m) m.remove();
  }
  ...
  var backdrop = el("div", { id: "potui2_backdrop" });
  var modal = el("div", { id: "potui2_modal" });

  var header = el("div", { id: "potui2_header" }, [
  ...
  var body = el("div", { id: "potui2_body" });

  var sidebar = el("div", { className: "p2sidebar" });
  var main = el("div", { className: "p2main" });
  ...
  sidebar.appendChild(el("div", { className: "warnBox", id: "potui2_core_warn", style: { display: "none" } }, [
    ...
    "（請確認 script 順序：先 potential_core_v2_es5.js，再 potential_ui...）"
  ]));
...
// 2) Tabs + slot (main)
  ...
  var _btnClose = modal.querySelector("#potui2_close");
  if (_btnClose) _btnClose.onclick = close;

  var _btnInv = modal.querySelector("#potui2_inv");
  if (_btnInv) _btnInv.onclick = function () {
    if (typeof w.openInventoryModal === "function") w.openInventoryModal();
  };

  var _btnProb = modal.querySelector("#potui2_prob_btn");
  if (_btnProb) _btnProb.onclick = openProbModal;

  var _btnAllFold = modal.querySelector("#potui2_all_fold");
...
function setAllSummaryMode(mode){
  ...
  var bAll = modal.querySelector("#potui2_all_f_all");
  var bPct = modal.querySelector("#potui2_all_f_pct");
  var bFlat = modal.querySelector("#potui2_all_f_flat");
...
var _bAll = modal.querySelector("#potui2_all_f_all");
var _bPct = modal.querySelector("#potui2_all_f_pct");
var _bFlat = modal.querySelector("#potui2_all_f_flat");
...
if (_bFlat) _bFlat.onclick = function(){ setAllSummaryMode("flat"); };
  ...
  var _btnSingleFold = modal.querySelector("#potui2_single_fold");
...
d.body.appendChild(backdrop);
  ...
  function potentialPanel(title, which) {
    ...
    var cbtn = card.querySelector("#potui2_" + which + "_combine");
  ...
  // Tabs
  // -----------------------------
  var _activeTabKey = "weapon";

  
function buildTabs() {
  var tabs = d.getElementById("potui2_tabs");
  if (!tabs) return;
  tabs.innerHTML = "";

  for (var i = 0; i < TABS.length; i++) {
    (function (tab) {
      var b = el("button", { className: "tabBtn", "data-tab": tab.key }, [tab.title]);
      b.onclick = function () {
...
function highlightTabs() {
  var st = coreReady() ? (safe(function () { return w.PotentialCoreV2.getState(); }, null) || {}) : null;

  var btns = d.querySelectorAll("#potui2_tabs .tabBtn");
  for (var i = 0; i < btns.length; i++) {
    var k = btns[i].getAttribute("data-tab");
    var tab = null;
    for (var t = 0; t < TABS.length; t++) if (TABS[t].key === k) tab = TABS[t];
    if (!tab) continue;
    ...
    var neonAll = tab.slots && tab.slots.length ? true : false;
    if (st && st.pots) {
      for (var s = 0; s < tab.slots.length; s++) {
        var info = getSlotTierInfo(st, tab.slots[s]);
  ...
  function getSelectedSlot() {
    ...
    var st = safe(function () { return w.PotentialCoreV2.getState(); }, null) || {};
...
function renderSlotsRow() {
  var wrap = d.getElementById("potui2_slots");
  if (!wrap) return;
  wrap.innerHTML = "";

  var tab = null;
  for (var i = 0; i < TABS.length; i++) if (TABS[i].key === _activeTabKey) tab = TABS[i];
  if (!tab) tab = TABS[0];
  ...
  for (var s = 0; s < tab.slots.length; s++) {
    (function (slot) {
      var info = getSlotTierInfo(st, slot);

      // 主色 = 主/附最高階；若主+附皆永恆 => neon（彩虹外框）
      var b = el("button", { className: "slotBtn " + tierClass(info.hi) + (info.neon ? " slotNeon" : ""), "data-slot": slot }, [
        // 左側小圓點：主/附
  ...
  function openRollModal(which) {
    ...
    safe(function () { w.PotentialCoreV2.ensureLinesExist(); }, null);

    closeRollModal();

    var slot = getSelectedSlot();
    var slotName = SLOT_LABEL[slot] || slot;

    var back = el("div", { id: "potui2_roll_backdrop" });
    var modal = el("div", { id: "potui2_roll_modal" });

    var titleTxt = (which === "add") ? "洗附加潛能" : "洗主潛能";
    var header = el("div", { id: "potui2_roll_header" }, [
      el("div", { className: "title" }, [
    ...
    var body = el("div", { id: "potui2_roll_body" });

    // state holders (in closure)
    var lastUpgraded = false;     // 上一次洗是否跳框
    var committedNode = null;    // 舊潛能（已套用在裝備上，整個彈窗期間不變）
    var previewNode = null;       // 新潛能（僅預覽，直到按「保留後」才會套用）
    var hasPreview = false;       // 是否已有預覽結果
    var hasRolledOnce = false;
    var flashyData = null;        // 閃炫候選資料
    var flashySel = null;         // 閃炫選擇狀態
    // 本次彈窗是否已經洗過（洗一次只出現第一次）

    function getCubeOptions() {
      var opt = { upChanceMult: 1, itemName: null, label: "", kind: "normal" };
      var PC = w.PotentialCoreV2 || w.PotentialCoreV1 || {};
      var sel = d.getElementById("potui2_roll_cubeSel");
      var v = sel ? String(sel.value || "normal") : "normal";
  ...
  function openCombineModal(which) {
    ...
    var slot = core.getSelectedSlot ? core.getSelectedSlot() : (core.getState().selectedSlot || "weapon");
    var st = core.getState ? core.getState() : null;
    var itemId = (which === "add") ? (core.ITEM_ADD_COMBINE || "ADD_COMBINE_CUBE") : (core.ITEM_MAIN_COMBINE || "MAIN_COMBINE_CUBE");
    ...
    var selectedLineIndex = null;
    var draw = null; // { ok, lineIndex, lineText, chanceSameFrame }
    var usedOnce = false;
    ...
    function render() {
      var qty = getQty();
      var title = (which === "add") ? "附加結合方塊" : "結合方塊";
      modal.innerHTML = "";

      // header
      var header = el("div", { className: "p2mHead" }, [
      ...
      // body
      var body = el("div", { className: "p2mBody" }, []);

      var btnUse = el("button", { className: "p2btn primary", id: "potui2_combine_use", disabled: qty <= 0 }, [
        usedOnce ? "再次使用（扣 1）" : "使用（扣 1）"
      ]);

      var hint = el("div", { className: "note", style: { marginTop: "10px" } }, [
      ...
      // result area
      var resultBox = el("div", { className: "p2card", style: { marginTop: "12px" } }, []);
      if (!draw) {
      ...
      var btnRow = el("div", { className: "btnRow", style: { marginTop: "12px" } }, [
      ...
      // handlers
      var cbtn = modal.querySelector("#potui2_combine_close");
      if (cbtn) cbtn.onclick = close;
      backdrop.onclick = function (e) { if (e.target === backdrop) close(); };

      var useBtn = modal.querySelector("#potui2_combine_use");
      var rerollBtn = modal.querySelector("#potui2_combine_reroll");
      var confirmBtn = modal.querySelector("#potui2_combine_confirm");

      function doDraw() {
        var res = (which === "add") ? core.drawAddCombine(slot) : core.drawMainCombine(slot);
      ...
      function doConfirm() {
        if (!draw) return;
        var res = (which === "add") ? core.confirmAddCombine(slot, draw.lineIndex) : core.confirmMainCombine(slot, draw.lineIndex);
...
function openFlashyModal(slot, which, opt) {
      ...
      var btnX = d.createElement("button");
      ...
      // state
      var lastRoll = null;
      var chosenMain = [];     // idx list 0..5
      var chosenAdd1 = 0;      // 0..1
      var chosenAdd23 = [];    // idx list 0..2
      var appliedOnce = false;
      var openedAt = Date.now(); 

      var committedBefore = null;
      function dc(o){ try { return JSON.parse(JSON.stringify(o)); } catch(_){ return o; } }
      function snapBefore(){
        if (committedBefore) return committedBefore;
        try {
          var st0 = (typeof core.getState === "function") ? core.getState() : null;
      ...
      function renderAppliedLines(lines) {
        ...
        for (var i = 0; i < lines.length; i++) {
          var r = makeLineRow(i, lines[i], true);
      ...
      function renderMainCandidates(cands) {
        ...
        for (var i = 0; i < cands.length; i++) {
          (function (idx) {
            var row = makeLineRow(idx, cands[idx], (chosenMain.indexOf(idx) >= 0));

            row.onclick = function () {
              if (appliedOnce) return;
              var k = chosenMain.indexOf(idx);
              if (k >= 0) {
      ...
      function renderAddCandidates(c1, c23) {
        ...
        // box1: 2選1
        var box1 = d.createElement("div");
        box1.className = "potui2_flashy_box";
        var t1 = d.createElement("h4");
        t1.textContent = "第1排（2選1）";
        box1.appendChild(t1);

        for (var i = 0; i < c1.length; i++) {
          (function (idx) {
            var row = makeLineRow(idx, c1[idx], false);

            row.onclick = function () {
              if (appliedOnce) return;
              chosenAdd1 = idx;
              var rows = box1.querySelectorAll(".potui2_flashy_line");
              for (var j = 0; j < rows.length; j++) {
                rows[j].classList.remove("sel");
                var ck = rows[j].querySelector(".potui2_flashy_check"); if (ck) ck.textContent = "";
              }
              row.classList.add("sel");
              row.querySelector(".potui2_flashy_check").textContent = "✓";
              btnOk.disabled = false;
        ...
        var t2 = d.createElement("h4");
        t2.textContent = "第2/3排（三選二）";
        box2.appendChild(t2);

        for (var k = 0; k < c23.length; k++) {
          (function (idx) {
            var row = makeLineRow(idx, c23[idx], (chosenAdd23.indexOf(idx) >= 0));

            row.onclick = function () {
              if (appliedOnce) return;
              var p = chosenAdd23.indexOf(idx);
              if (p >= 0) {
      ...
      function applySelection() {
        ...
        if (which === "add") {
          ...
          while (chosenAdd23.length < 2) {
            var cand = chosenAdd23.length; // 0,1
            if (chosenAdd23.indexOf(cand) < 0) chosenAdd23.push(cand);
            else break;
          }
          // 若還是不足（候選不足），直接擋下
          if (chosenAdd23.length !== 2) return;
          chosenAdd23.sort(function (a, b) { return a - b; });
          node.lines = [
          ...
          while (chosenMain.length < 3) {
            var next = chosenMain.length;
            if (chosenMain.indexOf(next) < 0) chosenMain.push(next);
            else break;
          }
          if (chosenMain.length !== 3) return;
          chosenMain.sort(function (a, b) { return a - b; });
          node.lines = [
      ...
      doRollFlashy();
        ...
        function hidePicker() {
      var box = d.getElementById("potui2_roll_picker");
    ...
    function showPickerMain(candidates) {
      ...
      function applySelection() {
        ...
        var chosen = [];
        for (var i3 = 0; i3 < flashyData.length; i3++) if (flashySel.picks[i3]) chosen.push(flashyData[i3]);
        if (chosen.length === 3) {
      ...
      function render() {
        box.innerHTML = "";
        box.appendChild(el("div", { className: "pickerTitle", text: "閃炫：一次抽 6 排，選擇要保留的 3 排" }));
        var list = el("div", { className: "pickerList" });
        for (var i = 0; i < flashyData.length; i++) {
          (function(ii){
            var chk = el("input", { type: "checkbox" });
            chk.checked = !!flashySel.picks[ii];
            chk.onchange = function(){
              var cur = 0;
              for (var k in flashySel.picks) if (flashySel.picks[k]) cur++;
              if (chk.checked) {
            ...
            var lineTxt = (flashyData[ii] && flashyData[ii].text) ? flashyData[ii].text : String(flashyData[ii]||"");
        ...
        var picked = [];
        for (var i2 = 0; i2 < flashyData.length; i2++) if (flashySel.picks[i2]) picked.push(i2+1);
    ...
    function showPickerAdd(cand) {
      ...
      function applySelection() {
        if (!previewNode) return;
        var l1 = flashyData.c1[flashySel.c1] || flashyData.c1[0];
        var chosen23 = [];
        for (var i = 0; i < 3; i++) if (flashySel.c23[i]) chosen23.push(flashyData.c23[i]);
      ...
      function render() {
        ...
        var g1 = el("div", { className: "pickerGroup" }, [ el("div", { className: "pickerSub", text: "第1排（2選1）" }) ]);
        for (var i1=0;i1<flashyData.c1.length;i1++){
          (function(ii){
            var r = el("input", { type:"radio", name:"potui2_c1" });
            r.checked = (flashySel.c1 === ii);
            r.onchange = function(){ flashySel.c1 = ii; applySelection(); };
            var t = flashyData.c1[ii] && flashyData.c1[ii].text ? flashyData.c1[ii].text : String(flashyData.c1[ii]||"");
            g1.appendChild(el("label", { className: "pickerRow" }, [ r, el("span", { className:"pickerLine", text: t }) ]));
          })(i1);
        }
        box.appendChild(g1);

        var g2 = el("div", { className: "pickerGroup" }, [ el("div", { className: "pickerSub", text: "第2/3排（三選二）" }) ]);
        for (var i2=0;i2<flashyData.c23.length;i2++){
          (function(ii){
            var chk = el("input", { type:"checkbox" });
            chk.checked = !!flashySel.c23[ii];
            chk.onchange = function(){
              var cur=0; for (var k=0;k<3;k++) if (flashySel.c23[k]) cur++;
              if (chk.checked){
            ...
            var t2 = flashyData.c23[ii] && flashyData.c23[ii].text ? flashyData.c23[ii].text : String(flashyData.c23[ii]||"");
            g2.appendChild(el("label", { className: "pickerRow" }, [ chk, el("span", { className:"pickerLine", text: t2 }) ]));
          })(i2);
        }
        box.appendChild(g2);

        var picked=[]; for (var j=0;j<3;j++) if (flashySel.c23[j]) picked.push(j+1);
...
function readNodeFromState(st) {
      st = st || safe(function () { return w.PotentialCoreV2.getState(); }, null) || {};
      var pots = (st && st.pots) ? st.pots : {};
      var node = (pots && pots[slot] && pots[slot][which]) ? pots[slot][which] : null;
      return safe(function(){ return JSON.parse(JSON.stringify(node || {})); }, {});
    }
    ...
    function updateCounts() {
      var opt = getCubeOptions();
      var q = safe(function () { return (typeof w.getItemQuantity === "function") ? n(w.getItemQuantity(opt.itemName)) : 0; }, 0);
      var c = d.getElementById("potui2_roll_count");
...
function getPityNeedUI(which, tier) {
  ...
  return 0;
    ...
    function updateRollPityUI(node) {
      var elP = d.getElementById("potui2_roll_pity");
      if (!elP) return;
      if (!node) { elP.textContent = "保底進度：—"; return; }
      var tier = String(node.tier || "特殊");
      var pity = Math.max(0, Math.floor(n(node.pity)));
      var need = getPityNeedUI(which, tier);
      if (!need) { elP.textContent = "保底進度：無保底"; return; }
      var remain = Math.max(0, need - (pity + 1));
    ...
    function render() {
      ...
      // compare grid
      var grid = el("div", { className: "rollGrid" });

      var beforeBox = el("div", { className: "rollBox", id: "potui2_roll_before_box" }, [
        el("h4", null, [
          el("span", { text: "洗之前" }),
      ...
      var afterBox = el("div", { className: "rollBox", id: "potui2_roll_after_box" }, [
        el("h4", null, [
          el("span", { text: "洗之後" }),
      ...
      // actions
      var btnRoll = el("button", { className: "p2btn pri grow", id: "potui2_roll_do" }, ["洗一次"]);
      var btnKeepBefore = el("button", { className: "p2btn", id: "potui2_roll_keep_before" }, ["原潛能"]);
      var btnKeepAfter = el("button", { className: "p2btn", id: "potui2_roll_keep_after" }, ["新潛能"]);
      var btnReroll = el("button", { className: "p2btn ghost", id: "potui2_roll_reroll" }, ["再洗一次"]);
      // 先隱藏「再洗一次」，第一次洗完後才顯示
      ...
      var flashyData = null; // stores latest candidates for flashy modes

      function hidePicker() {
        var picker = d.getElementById("potui2_roll_picker");
        var beforeBox = d.getElementById("potui2_roll_before_box");
        var afterBox = d.getElementById("potui2_roll_after_box");
      ...
      function renderFlashyPicker() {
        var picker = d.getElementById("potui2_roll_picker");
        var beforeBox = d.getElementById("potui2_roll_before_box");
        var afterLinesEl = d.getElementById("potui2_roll_after_lines");
        ...
        // after box title
        var afterBoxEl = d.getElementById("potui2_roll_after_box");
        var h4 = afterBoxEl ? afterBoxEl.querySelector("h4") : null;
        if (h4) {
          h4.innerHTML = "";
        ...
        if (which === "add") {
          ...
          for (var a = 0; a < c1.length; a++) (function(ix){
            ...
            var body = opt.querySelector(".flashyOptBody");
          ...
          var row23 = el("div", { className: "flashyChoices" });
          function pickC23(){
            var arr=[]; for (var z=0; z<3; z++) if (sel23[z]) arr.push(c23[z]);
            if (arr.length<2) { arr=[c23[0],c23[1]]; sel23={0:true,1:true,2:false}; }
            return arr.slice(0,2);
          }
          function enforceTwo(keepIx){
            var cnt=0; for (var z=0; z<3; z++) if (sel23[z]) cnt++;
            if (cnt>2) {
              // uncheck others except keepIx
              for (var z2=0; z2<3; z2++) if (z2!==keepIx && sel23[z2]) { sel23[z2]=false; break; }
            }
            if (cnt<2) {
              for (var z3=0; z3<3; z3++) if (!sel23[z3]) { sel23[z3]=true; cnt++; if (cnt===2) break; }
            }
          }
          for (var b = 0; b < c23.length; b++) (function(ix){
            var opt = el("label", { className: "flashyOpt" }, [
            ...
            opt.querySelector("input").onchange = function(){
              ...
              var ins = row23.querySelectorAll("input[type=checkbox]");
              for (var q=0;q<ins.length;q++){
                var v = parseInt(ins[q].value,10);
          ...
          var cands = flashyData.candidates || flashyData || [];
          var picked = {0:true,1:true,2:true,3:false,4:false,5:false};
          ...
          try {
            ...
            var cc=0; for (var j=0;j<6;j++) if (picked[j]) cc++;
          ...
          var row = el("div", { className: "flashyChoices" });

          function pickedArr(){
            var arr=[]; for (var i=0;i<6;i++) if (picked[i]) arr.push(cands[i]);
            if (arr.length!==3) {
          ...
          function enforceThree(keepIx){
            var cnt=0; for (var i=0;i<6;i++) if (picked[i]) cnt++;
            if (cnt>3) {
              for (var j=0;j<6;j++) if (j!==keepIx && picked[j]) { picked[j]=false; break; }
            }
            if (cnt<3) {
              for (var k=0;k<6;k++) if (!picked[k]) { picked[k]=true; cnt++; if (cnt===3) break; }
            }
          }

          for (var i=0;i<cands.length;i++) (function(ix){
            var opt = el("label", { className: "flashyOpt" }, [
              el("input", { type: "checkbox", value: String(ix) }, []),
              el("span", { className: "flashyOptBody" }, [])
            ]);
            ...
            opt.querySelector("input").onchange = function(){
              ...
              enforceThree(ix);
              // sync all
              var ins = row.querySelectorAll("input[type=checkbox]");
              for (var q=0;q<ins.length;q++){
                var v = parseInt(ins[q].value,10);
...
syncBtns();
      ...
      btnRoll.onclick = function () {
        ...
        // do roll
        var baseNode = hasPreview ? previewNode : committedNode;
...
if (!res || !res.ok) {
        ...
        // 保底：每洗一次 +1（與套用哪個結果無關）
        try {
          var needP = getPityNeedUI(which, String(committedNode.tier || "特殊"));
          var curP = Math.max(0, Math.floor(n(committedNode.pity)));
          if (lastUpgraded) {
            // 若跳框，保底重置（維持核心邏輯直覺）
        ...
        var tierAfter = String(previewNode.tier || "特殊");
        setText("potui2_roll_after_tier", "外框：" + tierAfter);
        setText("potui2_roll_before_tier", "外框：" + String(committedNode.tier || "特殊"));

        var badge = lastUpgraded ? el("span", { className: "badgeUp" }, ["跳框！"]) : null;
        var h4 = afterBoxEl ? afterBoxEl.querySelector("h4") : null;
        if (h4) {
          var mini = h4.querySelector(".mini");
          h4.innerHTML = "";
  ...
  function closeRollModal() {
    var b = d.getElementById("potui2_roll_backdrop");
    var m = d.getElementById("potui2_roll_modal");
    if (b) b.remove();
    if (m) m.remove();
  }
...
// -----------------------------
  ...
  function refresh() {
    ...
    var slot = getSelectedSlot();
    var slotName = SLOT_LABEL[slot] || slot;
    var selEl = d.getElementById("potui2_selected_name");
    ...
    // 核心就緒
    var st = safe(function () { return w.PotentialCoreV2.ensureLinesExist(); }, null)
      || safe(function () { return w.PotentialCoreV2.getState(); }, null)
      || {};

    // 聚合
    safe(function () { w.PotentialCoreV2.applySelectedToPlayer(w.player, slot); }, null);
    safe(function () { w.PotentialCoreV2.applyAllToPlayer(w.player); }, null);

    // auto tab (依照 TABS 定義，不再硬編 slot 類別)
    (function(){
      var found = null;
      for (var i = 0; i < TABS.length; i++) {
        var arr = TABS[i].slots || [];
  ...
  function renderAllSummaryPlaceholder() {
    ...
    var grid = d.getElementById("potui2_all_grid");
    if (!grid) return;
    grid.innerHTML = "";
  ...
  function renderSingleSummaryPlaceholder(slot) {
    ...
    var grid = d.getElementById("potui2_single_grid");
    if (!grid) return;
    grid.innerHTML = "";
    for (var i = 0; i < SUMMARY_KEYS.length; i++) {
      grid.appendChild(el("div", { className: "sumItem" }, [
...
function refreshAllSummary() {
    ...
    for (var slot in pots) {
      if (!pots.hasOwnProperty(slot)) continue;
      var p = pots[slot] || {};
      var mainLines = p.main && p.main.lines ? p.main.lines : [];
      var addLines  = p.add  && p.add.lines  ? p.add.lines  : [];
      for (var i = 0; i < mainLines.length; i++) consumeLine(mainLines[i]);
      for (var j = 0; j < addLines.length; j++) consumeLine(addLines[j]);
    }

    var pctOrder = ["bossDamage","eliteDamage","normalDamage","totalDamage","ignoreDef","critRate","critDamage","atk","str","agi","int","luk","hp","mp","def"];
    var flatOrder = ["atk","str","agi","int","luk","hp","mp","def"];
    ...
    var gridPct = d.getElementById("potui2_all_grid_pct");
    var gridFlat = d.getElementById("potui2_all_grid_flat");
    var secPct = d.getElementById("potui2_all_sec_pct");
    var secFlat = d.getElementById("potui2_all_sec_flat");
    var legacyGrid = d.getElementById("potui2_all_grid");

    if (legacyGrid && (!gridPct || !gridFlat)) {
      legacyGrid.innerHTML = "";
      var keys = sortKeys(uniqKeys(pctSum).concat(uniqKeys(flatSum)), pctOrder);
      var seen = {}, merged = [];
      for (var ii=0; ii<keys.length; ii++){ if(!seen[keys[ii]]){ seen[keys[ii]]=1; merged.push(keys[ii]); } }
      for (var kk=0; kk<merged.length; kk++){
        var key = merged[kk];
        var hasPct = (pctN[key] || 0) > 0;
        var vText = hasPct ? ("+" + fmtPct(pctSum[key], 2) + " (x" + pctN[key] + ")")
    ...
    var showPct = (allSummaryMode !== "flat");
    var showFlat = (allSummaryMode !== "pct");
    if (secPct) secPct.style.display = showPct ? "" : "none";
    if (secFlat) secFlat.style.display = showFlat ? "" : "none";

    gridPct.innerHTML = "";
    if (showPct) {
      var pctKeys = sortKeys(uniqKeys(pctSum), pctOrder);
      for (var ppi=0; ppi<pctKeys.length; ppi++){
        var s = pctKeys[ppi];
        if ((pctN[s]||0) <= 0) continue;
        var label = statLabelZH(s) || statLabel(s) || s;
        var vText = "+" + fmtPct(pctSum[s], 2) + " (x" + pctN[s] + ")";
    ...
    gridFlat.innerHTML = "";
    if (showFlat) {
      var flatKeys = sortKeys(uniqKeys(flatSum), flatOrder);
      for (var ffi=0; ffi<flatKeys.length; ffi++){
        var s2 = flatKeys[ffi];
        var vv = flatSum[s2] || 0;
        if (vv === 0) continue;
        var label2 = statLabelZH(s2) || statLabel(s2) || s2;
        var vText2 = fmtSignedInt(vv) + ((flatN[s2]||0)>1 ? (" (x" + flatN[s2] + ")") : "");
  ...
  function refreshSingleSummary(slot) {
    setText("potui2_single_hint", (SLOT_LABEL[slot] || slot) + "（主+附）");
    var sel = collectBonusForSlot(w.player, slot);
  ...
  function refreshPanel(which, st, slot) {
    var node = (st && st.pots && st.pots[slot] && st.pots[slot][which]) ? st.pots[slot][which] : { tier: "特殊", pity: 0, lines: [] };
    var tier = String(node.tier || "特殊");

    // frame
    var frame = d.getElementById("potui2_" + which + "_frame");
    var tierEl = d.getElementById("potui2_" + which + "_tier");
    ...
    // lines
    var wrap = d.getElementById("potui2_" + which + "_lines");
    if (wrap) {
      wrap.innerHTML = "";
      var lines = Array.isArray(node.lines) ? node.lines : [];
      for (var i = 0; i < 3; i++) {
        var L = lines[i] || null;
        var t = lineToText(L);        wrap.appendChild(el("div", { className: "line " + tierClass(t.tier) }, [
          el("div", { className: "l" }, [
            el("div", { className: "lineTop" }, [
              el("span", { className: "tierPill " + tierClass(t.tier) }, [
                el("span", { className: "dot" }),
    ...
    // cube count
    var cnt = d.getElementById("potui2_" + which + "_count");
    if (cnt) {
      var itemName = (which === "add") ? (w.PotentialCoreV2.ITEM_ADD || "附加方塊") : (w.PotentialCoreV2.ITEM_MAIN || "潛能方塊");
      var q = safe(function () { return (typeof w.getItemQuantity === "function") ? n(w.getItemQuantity(itemName)) : 0; }, 0);
      cnt.textContent = itemName + "：× " + q.toLocaleString();
    }
  }

  function refreshPityInPanel(which, tier, pityVal) {
    var pity = Math.max(0, Math.floor(n(pityVal)));
    var rule = getRuleUI(which, tier);
    var need = Math.max(0, Math.floor(n(rule.pity)));
    ...
    var remain = Math.max(0, need - (pity + 1)); // 核心判定是 pity+1>=need
    var pct = clamp((pity / need) * 100, 0, 100);
  ...
  function renderPanelPlaceholder(which) {
    ...
    if (wrap) {
      wrap.innerHTML = "";
      for (var i = 0; i < 3; i++) {        wrap.appendChild(el("div", { className: "line t_特殊" }, [
          el("div", { className: "l" }, [
            el("div", { className: "lineTop" }, [
  ...
  // Small setters
  // -----------------------------
  function setText(id, text) { var x = d.getElementById(id); if (x) x.textContent = text; }
  function setWidth(id, wstr) { var x = d.getElementById(id); if (x) x.style.width = wstr; }
  ...
  function close() {
    closeProbModal();
    var m = d.getElementById("potui2_modal");
    var b = d.getElementById("potui2_backdrop");
    if (m) m.remove();
    if (b) b.remove();
  }

  w.PotentialUIV2 = { open: open, close: close, refresh: refresh, openProb: openProbModal };
